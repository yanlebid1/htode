...
async def test_olx_url(url: str, proxy: Optional[str] = None, headless: bool = True):
    ...
    try:
        ...
        if result.phone_numbers:
            ...
            print("❌ FAILED: No phone numbers found")
...
async def _parse_olx_camoufox_debug(
    ad_url: str, proxy: Optional[str] = None, headless: bool = True
):
    ...
    print("🔍 Starting Camoufox debug session...")
    ...
    try:
        ...
        async with AsyncCamoufox(**camoufox_args) as browser:
            ...
            link_selector = (
                'div[data-testid="phones-container"] a[data-testid="contact-phone"]'
            )
            ...
            try:
                ...
                phone_links = await page.query_selector_all(
                    'div[data-testid="phones-container"] a[data-testid="contact-phone"]'
                )
                ...
                for i, link in enumerate(phone_links):
                    try:
                        ...
                        print(
                            f"  Link {i+1}: href='{href}' text='{text}' data-testid='{data_testid}'"
                        )
                    except Exception as e:
                        print(f"  Link {i+1}: <could not read attributes>: {e}")
...
async def main():
    ...
    print("📁 Check debug_screenshots/ folder for screenshots")
