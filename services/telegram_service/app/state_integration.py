...
class UnifiedFSMContextAdapter:
    ...
    async def get_data(self) -> dict:
        ...
        with log_context(self.logger, user_id=self.user_id, platform=self.platform):
            self.logger.debug(
                "Getting state data",
                extra={"user_id": self.user_id, "platform": self.platform},
            )
    ...
    async def get_state(self) -> str:
        ...
        with log_context(self.logger, user_id=self.user_id, platform=self.platform):
            ...
            state_name = await state_manager.get_current_state_name(
                self.platform, self.user_id
            )
            self.logger.debug(
                "State name retrieved",
                extra={
                    "user_id": self.user_id,
                    "platform": self.platform,
    ...
    async def reset_state(self, with_data: bool = True) -> None:
        ...
        with log_context(self.logger, user_id=self.user_id, platform=self.platform):
            ...
            if with_data:
                ...
                await state_manager.set_state(self.platform, self.user_id, current_data)
                self.logger.debug(
                    "State name cleared, data preserved",
                    extra={"user_id": self.user_id, "platform": self.platform},
                )
...
def get_state_for_user(user_id: str) -> UnifiedFSMContextAdapter:
    ...
    with log_context(logger, user_id=user_id):
        logger.debug(
            "Creating state adapter", extra={"user_id": user_id, "platform": "telegram"}
        )
    ...
    with log_context(logger, user_id=user_id):
        try:
            ...
            if data:
                ...
                logger.info("No state data to migrate", extra={"user_id": user_id})
        except Exception as e:
            logger.error(
                "Error migrating state",
                exc_info=True,
