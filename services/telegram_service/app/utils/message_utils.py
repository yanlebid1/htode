...
async def safe_send_message(
    chat_id: Union[int, str], text: str, **kwargs
) -> Optional[Message]:
    """
    ...
    with log_context(logger, chat_id=chat_id, text_length=len(text)):
        logger.debug(
            "Sending message directly through Telegram bot",
            extra={
                "chat_id": chat_id,
...
async def async_process_image_url(url: str) -> Union[str, InputFile, None]:
    ...
    with log_context(logger, url_length=len(url)):
        ...
        if not url or not isinstance(url, str):
            logger.warning(
                "Invalid image URL",
                extra={"url_type": type(url).__name__ if url else "None"},
            )
            return None
        ...
        try:
            ...
            if re.search(r"\.(jpg|jpeg|png|gif|webp)(\?|$)", parsed.path.lower()):
                logger.debug(
                    "URL appears to be a direct image link", extra={"url": url[:100]}
                )
                return url
            ...
            try:
                async with aiohttp.ClientSession(
                            ...
                            if "image/" in content_type:
                                if "image/webp" in content_type:
                                    ...
                                    try:
                                        ...
                                        logger.error(
                                            "Failed to convert WEBP to JPEG",
                                            exc_info=True,
                                            extra={"error": str(e)},
                                        )
                                        return None
                                logger.debug(
                                    "URL confirmed as image through HEAD request",
                    ...
                    async with session.get(url) as img_resp:
                        if img_resp.status == 200 and "image/" in img_resp.headers.get(
                            "Content-Type", ""
                        ):
                            data = await img_resp.read()
                ...
                logger.error(
                    "Error processing image URL asynchronously",
                    exc_info=True,
                    extra={"error": str(e)},
                )
        ...
        try:
            ...
            if isinstance(photo, str):
                ...
                if not processed_photo:
                    ...
                    send_kwargs["parse_mode"] = (
                        "Markdown"  # Force Markdown for the image link
                    )

                    return await safe_send_message(
                        chat_id=chat_id, text=full_text, **send_kwargs
                    )
                photo = processed_photo

            logger.debug(
                "Sending photo via direct bot method",
                extra={
                    "chat_id": chat_id,
            ...
            if isinstance(result, Message) and result.photo:
                logger.info("Photo sent successfully", extra={"chat_id": chat_id})
...
async def safe_edit_message(
    chat_id: Union[int, str], message_id: int, text: str, **kwargs
) -> Optional[Message]:
...
async def delete_message_safe(chat_id: Union[int, str], message_id: int) -> bool:
    """Telegram-specific wrapper for delete_message utility."""
    with log_context(logger, chat_id=chat_id, message_id=message_id):
        logger.debug(
            "Deleting message", extra={"chat_id": chat_id, "message_id": message_id}
        )
