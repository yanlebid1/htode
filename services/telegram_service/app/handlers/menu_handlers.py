...
from common.db.operations import (
    ...
    Ad,
    add_subscription as add_user_subscription,
)
...
async def edit_parameters(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    with log_context(logger, user_id=user_id, callback_data=callback_query.data):
        logger.info("User requested parameter editing", extra={"user_id": user_id})
...
async def handle_edit_button(message: types.Message):
    ...
    with log_context(logger, user_id=user_id, button_text=message.text):
        logger.info("Edit button pressed", extra={"user_id": user_id})
...
async def subscription_back_handler(callback_query: types.CallbackQuery):
    ...
    with log_context(logger, user_id=user_id, callback_data=callback_query.data):
        logger.info(
            "Returning to main menu from subscription", extra={"user_id": user_id}
        )
...
async def how_to_use_handler(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
    with log_context(logger, user_id=user_id, callback_data=callback_query.data):
        logger.info("Showing how to use information", extra={"user_id": user_id})
...
async def contact_support_handler(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
    with log_context(logger, user_id=user_id, callback_data=callback_query.data):
        logger.info("User wants to contact support", extra={"user_id": user_id})
...
async def menu_tech_support_handler(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
    with log_context(logger, user_id=user_id, callback_data=callback_query.data):
        logger.info("Showing tech support menu", extra={"user_id": user_id})
...
async def back_to_main_menu_handler(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
    with log_context(logger, user_id=user_id, callback_data=callback_query.data):
        logger.info("Returning to main menu", extra={"user_id": user_id})
...
async def subscribe(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    with log_context(logger, telegram_id=telegram_id, user_db_id=user_db_id):
        ...
        if not user_db_id:
            ...
            logger.warning(
                "user_db_id not found in state, trying to retrieve from DB",
        ...
        if not user_db_id:
            logger.error(
                "Failed to get or create user", extra={"telegram_id": telegram_id}
            )
            await safe_send_message(
                chat_id=callback_query.from_user.id,
        ...
        if not user_exists:
            logger.error(
                ...
                extra={"user_db_id": user_db_id, "telegram_id": telegram_id},
            )
            ...
            if not user_exists:
                logger.error(
                    "Failed to create user even after retry",
                    extra={"telegram_id": telegram_id},
                )
                await safe_send_message(
                    chat_id=callback_query.from_user.id,
        ...
        try:
            ...
            logger.info(
                "Subscription added successfully", extra={"user_db_id": user_db_id}
            )
            start_free_subscription_of_user(user_db_id)
            logger.info("Free subscription started", extra={"user_db_id": user_db_id})
        except Exception as e:
            logger.error(
                "Error updating user filters",
                exc_info=True,
                extra={"user_db_id": user_db_id, "filters": filters, "error": str(e)},
            )
            await safe_send_message(
                chat_id=telegram_id,
        ...
        await safe_send_message(
            chat_id=telegram_id, text="Ви успішно підписалися на пошук оголошень!"
        )

        # 2) Now do the multi-step check in local DB
        logger.info(
            "Starting to fetch ads for period", extra={"user_db_id": user_db_id}
        )
        final_ads = []
        for days_limit in [1, 3, 7, 14, 30]:
            ads = fetch_ads_for_period(filters, days_limit, limit=3)
            logger.info(
                f"Fetched ads for {days_limit} days",
                extra={
                    "user_db_id": user_db_id,
...
async def unsubscribe_callback(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    with log_context(logger, user_id=user_id):
        logger.info("User unsubscribing", extra={"user_id": user_id})
        ...
        try:
            ...
            logger.info("User subscription deactivated", extra={"user_id": user_id})
        except Exception as e:
            logger.error(
        ...
        await safe_send_message(
            chat_id=user_id, text="Ви відписалися від розсилки оголошень."
        )
        await safe_answer_callback_query(callback_query.id)
...
async def cancel_handler(message: types.Message, state: FSMContext):
    ...
    with log_context(logger, user_id=user_id):
        ...
        if current_state is None:
            logger.info("Cancel called but no state active", extra={"user_id": user_id})
            return
            ...
            with db_session() as db:
                ...
                for ad in ads:
                    ad_dict = {
                        ...
                        "insert_time": (
                            ad.insert_time.isoformat() if ad.insert_time else None
                        ),
                ...
                logger.info(
                    ...
                    extra={"ads_count": len(result), "days": days, "filters": filters},
                )
                return result
        except Exception as e:
            logger.error(
                "Error fetching ads for period",
                exc_info=True,
                extra={"filters": filters, "days": days, "error": str(e)},
            )
            return []
...
async def handle_how_to_use(message: types.Message):
    user_id = message.from_user.id
    with log_context(logger, user_id=user_id):
        logger.info("User requested how-to-use info", extra={"user_id": user_id})
        ...
        bot_msg = await safe_send_message(
            chat_id=message.from_user.id, text=text, reply_markup=how_to_use_keyboard()
        )
...
async def handle_back(message: types.Message, state: FSMContext):
    ...
    with log_context(logger, user_id=user_id):
        logger.info(
            ...
            extra={"user_id": user_id, "fsm_state": await state.get_state()},
        )
        ...
        try:
            ...
            logger.debug(
                "BACK_INFO not available or error cleaning", extra={"error": str(e)}
            )
...
async def add_subscription_prompt(message: types.Message):
    ...
    user_id = message.from_user.id
    with log_context(logger, user_id=user_id):
        logger.info("User wants to add subscription", extra={"user_id": user_id})
...
async def send_main_menu(chat_id: int):
    ...
    msg = await safe_send_message(
        chat_id=chat_id, text="Головне меню:", reply_markup=main_menu_keyboard()
    )
