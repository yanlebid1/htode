...
class VerificationRepository(BaseRepository):
    ...
    def create_verification(
        ...
        with log_context(
            ...
            if deleted > 0:
                logger.debug(
                    "Deleted existing verifications",
                    extra={
                        "target": target[:5] + "...",
    ...
    def verify_code(
        db: Session, verification_type: str, target: str, code: str
    ) -> bool:
        """Verify a code/token for phone or email"""
        with log_context(
            logger, verification_type=verification_type, target=target[:5] + "..."
        ):
            ...
            if verification:
                ...
                if verification.user_id:
                    user = (
                        db.query(User).filter(User.id == verification.user_id).first()
                    )
                    if user:
                        if verification_type == "phone":
    ...
    def get_active_verification(
        db: Session, verification_type: str, target: str
    ...
    def cleanup_expired_verifications(db: Session) -> int:
        ...
        with log_context(logger):
            ...
            logger.info(
                "Cleaned up expired verifications", extra={"deleted_count": result}
            )
    ...
    def get_recent_verification_attempts(
        db: Session, verification_type: str, target: str, minutes: int = 30
    ) -> int:
        ...
        with log_context(
            ...
            logger.debug(
                "Counted recent verification attempts",
                extra={
                    "type": verification_type,
                    "target": target[:5] + "...",
                    "minutes": minutes,
    ...
    def mark_user_verified(
        db: Session, user_id: int, verification_type: str, target: str
    ) -> bool:
    ...
    def get_user_verifications(
        db: Session, user_id: int, verification_type: Optional[str] = None
    ) -> List[Verification]:
