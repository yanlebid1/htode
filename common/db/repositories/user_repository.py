...
class UserRepository:
    ...
    def get_by_messenger_id(
        ...
        with log_context(
            ...
            if user:
                ...
                logger.debug(
                    "User not found by messenger ID",
                    extra={
                        "messenger_type": messenger_type,
                        "messenger_id": messenger_id,
                    },
    ...
    def get_by_email(db: Session, email: str) -> Optional[User]:
        ...
        with log_context(logger, email=email[:5] + "..."):
            ...
            if user:
                ...
                logger.debug(
                    "User not found by email", extra={"email": email[:5] + "..."}
                )
    ...
    def start_free_subscription(db: Session, user_id: int) -> bool:
        ...
        with log_context(logger, user_id=user_id):
            ...
            if not user:
                logger.warning(
                    "Cannot start free subscription - user not found",
    ...
    def get_subscription_status(db: Session, user_id: int) -> Dict[str, Any]:
        ...
        with log_context(logger, user_id=user_id):
            ...
            if not user:
                logger.warning(
                    "Cannot get subscription status - user not found",
                    extra={"user_id": user_id},
                )
                return {"active": False}
    ...
    def update_last_active(db: Session, user_id: int) -> bool:
        ...
        with log_context(logger, user_id=user_id):
            ...
            if not user:
                logger.warning(
                    "Cannot update last active - user not found",
                    extra={"user_id": user_id},
                )
                return False
    ...
    def create_messenger_user(
        ...
        with log_context(
            ...
            user_data = {messenger_field: messenger_id, "free_until": free_until}
            user = User(**user_data)
            db.add(user)
            db.commit()
            db.refresh(user)
    ...
    def update_messenger_id(
        ...
        with log_context(
            ...
            if not user:
                logger.warning(
                    "Cannot update messenger ID - user not found",
                    extra={"user_id": user_id},
                )
                return False
    ...
    def link_phone_number(
        ...
        with log_context(
            logger, user_id=user_id, phone_number=phone_number[:5] + "..."
        ):
            user = UserRepository.get_by_id(db, user_id)
            if not user:
                logger.warning(
                    "Cannot link phone number - user not found",
                    extra={"user_id": user_id},
                )
                return False
    ...
    def get_users_with_expiring_subscription(db: Session, days: int) -> List[User]:
        ...
        with log_context(logger, days=days):
            ...
            logger.debug(
                ...
                extra={"days": days, "user_count": len(users)},
            )
    ...
    def get_active_users(db: Session, days: int = 7, limit: int = 100) -> List[User]:
        ...
        with log_context(logger, days=days, limit=limit):
            ...
            logger.debug(
                "Found active users",
                extra={"days": days, "limit": limit, "user_count": len(users)},
            )
    ...
    def update_subscription_end_date(
        ...
        with log_context(
            logger, user_id=user_id, subscription_until=subscription_until.isoformat()
        ):
            user = UserRepository.get_by_id(db, user_id)
            if not user:
                logger.warning(
                    "Cannot update subscription end date - user not found",
                    extra={"user_id": user_id},
                )
                return False
    ...
    def get_user_id_by_telegram_id(db: Session, telegram_id: str) -> Optional[int]:
        ...
        with log_context(logger, telegram_id=telegram_id):
            ...
            if user:
                logger.debug(
                    "Found user ID by telegram ID",
                    extra={"telegram_id": telegram_id, "user_id": user.id},
                )
                return user.id
            else:
                logger.debug(
                    "User ID not found by telegram ID",
    ...
    def get_subscription_until(
        db: Session, user_id: int, free: bool = False
    ) -> Optional[str]:
        ...
        with log_context(logger, user_id=user_id, free=free):
            ...
            if not user:
                logger.debug(
                    "User not found for subscription until", extra={"user_id": user_id}
                )
                return None
            ...
            if date_field:
                ...
                logger.debug(
                    "Retrieved subscription until date",
                    extra={"user_id": user_id, "free": free, "date": result},
                )
                return result

            logger.debug(
                "No subscription until date found",
