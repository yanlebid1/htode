...
class EmailVerificationRepository:
    ...
    def create_token(
        db: Session, email: str, token: str, expires_at: datetime
    ) -> EmailVerificationToken:
        """Create a verification token for an email"""
        with log_context(
            logger, email=email[:5] + "...", expires_at=expires_at.isoformat()
        ):
            # Delete any existing tokens for this email
            deleted_count = (
                db.query(EmailVerificationToken)
                .filter(EmailVerificationToken.email == email)
                .delete()
            )

            if deleted_count > 0:
                logger.debug(
                    "Deleted existing tokens",
                    extra={"email": email[:5] + "...", "deleted_count": deleted_count},
                )

            # Create a new token
            token_record = EmailVerificationToken(
                email=email, token=token, expires_at=expires_at, attempts=0
            )
            db.add(token_record)
            db.commit()
    ...
    def get_token(db: Session, email: str) -> Optional[EmailVerificationToken]:
        ...
        with log_context(logger, email=email[:5] + "..."):
            ...
            if token:
                ...
                logger.debug(
                    "No verification token found", extra={"email": email[:5] + "..."}
                )
    ...
    def increment_attempts(db: Session, token_id: int) -> int:
        ...
        with log_context(logger, token_id=token_id):
            ...
            if token:
                ...
                logger.debug(
                    "Incremented token attempts",
                    extra={"token_id": token_id, "new_attempts": token.attempts},
                )

                return token.attempts

            logger.warning(
                "Token not found for increment", extra={"token_id": token_id}
            )
            return 0
    ...
    def verify_token(db: Session, email: str, token: str) -> bool:
        ...
        with log_context(logger, email=email[:5] + "..."):
            ...
            if not token_record:
                logger.warning(
                    "Invalid or expired token", extra={"email": email[:5] + "..."}
                )
                return False

            # Increment attempt counter
            attempts = EmailVerificationRepository.increment_attempts(
                db, token_record.id
            )
    ...
    def delete_token(db: Session, email: str) -> bool:
        ...
        with log_context(logger, email=email[:5] + "..."):
            ...
            logger.info(
                ...
                extra={"email": email[:5] + "...", "deleted_count": result},
            )
        ...
        with log_context(logger, email=email[:5] + "..."):
            ...
            if user:
                ...
                logger.info(
                    ...
                    extra={"user_id": user.id, "email": email[:5] + "..."},
                )

                return True

            logger.warning(
                "User not found for email verification",
                extra={"email": email[:5] + "..."},
            )
            return False
