...
class BaseRepository(Generic[T]):
    ...
    def get_by_id(db: Session, model_class: Type[T], id: int) -> Optional[T]:
        ...
        with log_context(logger, model=model_class.__name__, id=id):
            ...
            if entity:
                logger.debug(
                    "Found entity", extra={"model": model_class.__name__, "id": id}
                )
            else:
                logger.debug(
                    "Entity not found", extra={"model": model_class.__name__, "id": id}
                )
            return entity
    ...
    def get_all(db: Session, model_class: Type[T]) -> List[T]:
        ...
        with log_context(logger, model=model_class.__name__):
            ...
            logger.debug(
                "Retrieved all entities",
                extra={"model": model_class.__name__, "count": len(entities)},
            )
            return entities
    ...
    def update(db: Session, entity: T, data: dict) -> T:
        ...
        with log_context(
            logger, model=entity.__class__.__name__, id=getattr(entity, "id", None)
        ):
