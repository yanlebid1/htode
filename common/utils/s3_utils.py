    ...
    with log_context(logger, image_url=image_url[:50]):
        ...
        try:
            ...
            if (
                ...
                and AWS_CONFIG["cloudfront_domain"] in image_url
            ):
                ...
                s3_key = image_url.split(
                    f"{AWS_CONFIG['s3_bucket']}.s3.amazonaws.com/"
                )[-1]
            else:
                logger.warning(
                    "URL doesn't match expected CloudFront or S3 pattern",
                    extra={
            ...
            try:
                s3_client.delete_object(Bucket=AWS_CONFIG["s3_bucket"], Key=s3_key)
                logger.info(
                    "Successfully deleted image from S3",
                    extra={"s3_key": s3_key, "bucket": AWS_CONFIG["s3_bucket"]},
                )
                return True
            ...
            logger.error(
                "Unexpected error deleting image from S3",
                exc_info=True,
                extra={"image_url": image_url[:50], "error_type": type(e).__name__},
            )
            return False
    ...
    with log_context(logger, image_count=len(image_urls)):
        ...
        for image_url in image_urls:
            try:
                ...
                logger.error(
                    "Error processing batch delete item",
                    exc_info=True,
                    extra={"image_url": image_url[:50], "error_type": type(e).__name__},
                )
...
def detect_content_type(image_url, file_extension):
    ...
    with log_context(logger, file_extension=file_extension, image_url=image_url[:50]):
        ...
        logger.debug(
            ...
            extra={"content_type": content_type, "file_extension": file_extension},
        )
...
def _upload_image_to_s3(image_url, ad_unique_id, max_retries=3, retry_delay=1):
    ...
    with log_context(logger, image_url=image_url[:50], ad_id=ad_unique_id):
        ...
        logger.info(
            ...
            extra={"image_url": image_url[:50], "ad_id": ad_unique_id},
        )
        ...
        try:
            ...
            try:
                ...
                s3_key = (
                    f"{AWS_CONFIG['s3_prefix']}{ad_unique_id}_{hash(image_url)}.jpg"
                )
            ...
            if file_extension == "webp":
                try:
                    ...
                    logger.warning(
                        "Failed to convert WEBP to JPEG, uploading as is",
                        extra={"error": str(conv_err)},
                    )
            ...
            for attempt in range(max_retries):
                try:
                    with log_context(logger, attempt=attempt + 1, s3_key=s3_key):
                        ...
                        logger.debug(
                            "Successfully uploaded to S3", extra={"s3_key": s3_key}
                        )
            ...
            if AWS_CONFIG["cloudfront_domain"]:
                ...
                final_url = (
                    f"https://{AWS_CONFIG['s3_bucket']}.s3.amazonaws.com/{s3_key}"
                )

            logger.info(
                "Successfully uploaded image",
                extra={"final_url": final_url[:50], "s3_key": s3_key},
            )
