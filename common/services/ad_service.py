...
class AdService:
    ...
    def process_and_insert_ad(
        ...
        property_type: str,
        geo_id: int,
        extract_phones_sync: bool = False,
    ) -> Optional[int]:
        """
        ...
        with log_context(
            ...
            if not ad_unique_id:
                logger.warning(
                    "Missing ad ID, skipping insertion", extra={"ad_data": ad_data}
                )
                return None
            ...
            try:
                ...
                try:
                    ...
                    db.rollback()
                    logger.warning(
                        "Duplicate ad detected during insert, fetching existing",
                        extra={"external_id": ad_unique_id},
                    )
                ...
                logger.info(
                    "Created new ad",
                    extra={"ad_id": ad_id, "external_id": ad_unique_id},
                )
                ...
                if uploaded_image_urls:
                    ...
                    logger.info(
                        "Processed and inserted images",
                        extra={"ad_id": ad_id, "image_count": len(uploaded_image_urls)},
                    )

                # Extract phone numbers based on mode
                if extract_phones_sync:
                    # Synchronous extraction - wait for completion
                    try:
                        ...
                        if result["status"] == "success":
                            ...
                            if parser:
                                soup = BeautifulSoup(
                                    result.get("content", ""), "html.parser"
                                )
                                phone_result = parser.extract_phones(final_url, soup)
                            else:
                                from common.utils.phone_utils.phone_models import (
                                    ExtractionResult,
                                )
                            ...
                            # Store phones immediately
                            for phone in phone_result.phone_numbers:
                                AdRepository.add_phone(db, ad_id, phone)

                            if phone_result.viber_link:
                                AdRepository.add_phone(
                                    db, ad_id, None, phone_result.viber_link
                                )

                            logger.info(
                                "Phone extraction completed synchronously",
                                extra={
                                    "ad_id": ad_id,
                            ...
                            logger.warning(
                                ...
                                extra={"ad_id": ad_id, "error": result.get("error")},
                            )

                    except Exception as e:
                        logger.error(
                            "Error during synchronous phone extraction",
                            exc_info=True,
                            extra={"ad_id": ad_id, "error_type": type(e).__name__},
                        )
                else:
                    # Async extraction - schedule for later
                    ...
                    logger.info(
                        "Scheduled phone extraction",
                        extra={"ad_id": ad_id, "resource_url": resource_url},
                    )
    ...
    def cleanup_old_ads(
        db: Session, days_old: int = 30, check_activity: bool = True
    ) -> Tuple[int, int]:
        """
