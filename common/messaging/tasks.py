    ...
    with log_context(logger, user_id=user_id, ad_id=ad_id):
        ...
        async def _async_send():
            ...
            if image_urls:
                ...
                markup.add(InlineKeyboardButton("🖼 Більше фото", web_app=WebAppInfo(url=f"https://f3cc-178-150-42-6.ngrok-free.app/gallery?images={imgs}")))

            if phone_list:
                phones = ",".join(phone_list)
                markup.add(InlineKeyboardButton("📲 Подзвонити", web_app=WebAppInfo(url=f"https://f3cc-178-150-42-6.ngrok-free.app/phones?numbers={phones}")))
            ...
            try:
                ...
                logger.info("Ad sent successfully", extra={"telegram_id": telegram_id, "ad_id": ad_id})
                return True
            except Exception as e:
                logger.error("Failed to send ad", exc_info=True, extra={"error_type": type(e).__name__})
                return False
