...
async def test_safe_send_message_success():
    ...
    with patch(
        "services.telegram_service.app.bot.bot.send_message", new_callable=AsyncMock
    ) as mock_send:
...
async def test_safe_send_message_retry():
    ...
    with patch(
        ...
        mock_send.side_effect = [Exception("Network error"), MagicMock()]
...
async def test_safe_send_photo():
    ...
    with patch(
        "services.telegram_service.app.bot.bot.send_photo", new_callable=AsyncMock
    ) as mock_send:
        mock_send.return_value = MagicMock()
...
async def test_safe_answer_callback_query():
    ...
    with patch(
        "services.telegram_service.app.bot.bot.answer_callback_query",
...
async def test_telegram_handler_start(test_telegram_message):
    ...
    with patch(
        "services.telegram_service.app.utils.message_utils.safe_send_message",
        new_callable=AsyncMock,
    ) as mock_send:
...
async def test_send_ad_with_extra_buttons():
    ...
    with patch(
        "services.telegram_service.app.tasks.safe_send_photo", new_callable=AsyncMock
    ) as mock_send_photo, patch(
        "services.telegram_service.app.tasks.safe_send_message", new_callable=AsyncMock
    ), patch(
        "asyncio.run"
    ) as mock_run:
